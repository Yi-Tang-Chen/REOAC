...
def _save_checkpoint(
    ...
    torch.save(actor.state_dict(), os.path.join(ckpt_dir, "actor.pt"))
    torch.save(critic.state_dict(), os.path.join(ckpt_dir, "critic.pt"))

    if finetune_mode == "lora":
        lora_path = os.path.join(ckpt_dir, "backbone_lora")
        lora_manager.save_lora(backbone.model, lora_path)
    else:
        torch.save(backbone.model.state_dict(), os.path.join(ckpt_dir, "backbone.pt"))


def train(config_path: str, finetune_mode_override: str | None = None) -> None:
    # Silence known HF MDLM FutureWarnings about torch.cuda.amp.autocast deprecation.
    ...
    buffer = RolloutBuffer(capacity=int(config.get("buffer_size", 1000)))
    ...
    progress = _tqdm(total=total_episodes, desc="rollout", unit="episode", disable=total_episodes <= 1)
    ...
    for iteration in range(num_iterations):
        ...
        for step in range(episodes_per_iter):
            sample = dataset[(iteration * episodes_per_iter + step) % len(dataset)]
            ...
            sample_path = os.path.join(run_dir, "samples", f"iter{iteration}_step{step}.json")
